...
function SettingsPopup({ price, setPrice, lD8, setLD8, lD9, setLD9, onClose }) {
  return (
    ...
    >
      <div
        className="panel rounded-xl p-5 w-80 shadow-2xl space-y-4"
      ...
      >
        ...
        <label className="block">
          <span className="text-xs text-muted thai">
            Diesel #8 (เกาะ A) — อัตราสิ้นเปลือง (ลิตร/kWh)
          </span>
          <input
            type="number"
            min={0}
        ...
        <label className="block">
          <span className="text-xs text-muted thai">
            Diesel #9 (เกาะ C) — อัตราสิ้นเปลือง (ลิตร/kWh)
          </span>
          <input
            type="number"
            min={0}
  ...
  return (
    <section className="panel rounded-xl p-5">
      ...
      <div className="text-[10px] text-muted thai mb-3 flex items-center gap-3 flex-wrap">
        <span>
          ราคา: <span className="mono">{p}</span> ฿/ลิตร
        </span>
        <span>
          Diesel #8: <span className="mono">{ld8}</span> ลิตร/kWh
        </span>
        <span>
          Diesel #9: <span className="mono">{ld9}</span> ลิตร/kWh
        </span>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="panel rounded-lg p-3">
          <div className="text-[10px] uppercase eyebrow text-muted thai">
            Diesel #8 (เกาะ A)
          </div>
          <div
            className="text-lg mono font-bold mt-1"
            style={{ color: "#8b5cf6" }}
          >
            {fmtL(a)} <span className="text-xs text-muted">ลิตร</span>
          </div>
        </div>
        <div className="panel rounded-lg p-3">
          <div className="text-[10px] uppercase eyebrow text-muted thai">
          ...
          <div
            ...
            style={{ color: "#ef4444" }}
          >
            {fmtL(c)} <span className="text-xs text-muted">ลิตร</span>
          </div>
        </div>
        <div className="panel rounded-lg p-3">
          <div className="text-[10px] uppercase eyebrow text-muted thai">
          ...
          <div className="text-lg mono font-bold mt-1">
            {fmtL(total)} <span className="text-xs text-muted">ลิตร</span>
          </div>
        </div>
        <div className="panel rounded-lg p-3">
          <div className="text-[10px] uppercase eyebrow text-muted thai">
      ...
      {total < 1 && (
        <div
          className="mt-3 rounded-lg px-3 py-2 text-sm thai"
          style={{
