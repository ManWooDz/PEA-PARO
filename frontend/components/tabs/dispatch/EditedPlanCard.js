...
function Delta({ value, unit }) {
  ...
  const up = value > 0;
  const text =
    unit === "baht" ? fmtBaht(Math.abs(value)) : fmtL(Math.abs(value));
  return (
    <span
...
export function EditedPlanCard({ recommended, edited, warnings }) {
  ...
  const dLitres =
    (edited.diesel_litres ?? 0) - (recommended?.diesel_litres ?? 0);

  return (
    <section className="panel rounded-xl p-5 mt-4">
      <div className="text-base font-semibold thai mb-3">
        📝 แผนที่แก้ไข · เทียบแผนแนะนำ
      </div>
        ...
        <tbody>
          <tr className="border-b hairline">
            ...
            <td className="py-2 px-2 text-right mono">
              {fmtBaht(recommended?.total_thb)}
            </td>
            <td className="py-2 px-2 text-right mono font-semibold">
              {fmtBaht(edited.total_thb)}
            </td>
            <td className="py-2 pl-2 text-right">
              <Delta value={dCost} unit="baht" />
            </td>
          </tr>
          <tr>
            <td className="py-2 pr-3 thai">น้ำมันดีเซลรวม</td>
            <td className="py-2 px-2 text-right mono">
              {fmtL(recommended?.diesel_litres)}
            </td>
            <td className="py-2 px-2 text-right mono font-semibold">
              {fmtL(edited.diesel_litres)}
            </td>
            <td className="py-2 pl-2 text-right">
              <Delta value={dLitres} unit="litres" />
            </td>
          </tr>
        </tbody>
      </table>
